...
export const ProductRegisterForm = ({ onSuccessSubmit }: ProductFormProps) => {
  ...
  // Hooks de React Query
  const { createProduct, mutateUpdate, isCreating, isUpdating } =
    useProductMutations();
  ...
  const handleIncrement = (
    field: "stock" | "stockMinimo" | "precioVenta" | "precioCoste" | "discount",
  ) => {
    const currentValue = parseFloat(formData[field].toString()) || 0;
    const increment = ["precioVenta", "precioCoste", "discount"].includes(field)
      ? 0.1
      : 1;
    ...
    if (!previewToRemove.isLocal) {
      ...
      setFormData((prev) => ({
        ...
        imagenes: prev.imagenes.filter(
          (_, i) => i !== previewToRemove.fileIndex,
        ),
      }));
      setPreviews((prev) => prev.filter((p) => p.id !== previewToRemove.id));
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!formData.nombre || !formData.codigo || !formData.precioVenta) {
      await Swal.fire({ icon: "warning", title: "Campos requeridos", text: "Por favor completa los campos requeridos (*)." });

      return;
    }

    if (previews.length === 0) {
      await Swal.fire({ icon: "warning", title: "Imagen requerida", text: "Debes agregar al menos una imagen del producto." });
    ...
    try {
      if (isEditMode && productId) {
        ...
        await Swal.fire({ icon: "success", title: "¡Actualizado!", text: "Producto actualizado exitosamente." });
      } else {
        await createProduct({ dataProduct: dataToSubmit });
        await Swal.fire({ icon: "success", title: "¡Registrado!", text: "Producto registrado exitosamente." });
      ...
      await Swal.fire({ icon: "error", title: "Error", text: "Ocurrió un error al procesar la transacción." });
    }
  };
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                    <div>
                      ...
                      <Select
                        ...
                        disabled={isSubmitting}
                      >
                        <SelectTrigger className="w-full border-2 border-gray-300 focus:border-blue-500 rounded-lg">
                    ...
                    <div>
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <Input
                          ...
                          onChange={(e) =>
                            handleIntegerChange("stock", e.target.value)
                          }
                    ...
                    <div>
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <Input
                          ...
                          onChange={(e) =>
                            handleIntegerChange("stockMinimo", e.target.value)
                          }
                          className="border-2 border-gray-300 focus:border-blue-500 rounded-lg text-center flex-1"
                          disabled={isSubmitting}
                        />
                    ...
                    <div>
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <Input
                          ...
                          onChange={(e) =>
                            handleInputChange("precioVenta", e.target.value)
                          }
                          className="border-2 border-gray-300 focus:border-blue-500 rounded-lg text-center flex-1"
                          disabled={isSubmitting}
                        />
                    ...
                    <div>
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <Input
                          ...
                          onChange={(e) =>
                            handleInputChange("precioCoste", e.target.value)
                          }
                          className="border-2 border-gray-300 focus:border-blue-500 rounded-lg text-center flex-1"
                          disabled={isSubmitting}
                        />
