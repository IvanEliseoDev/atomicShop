  ...
  const fetchPurchaseHistory = async () => {
    try {
      ...
      if (response.ok && resData.data) {
        const mapped: PurchaseItem[] = resData.data.map((inv: any) => ({
          id: inv._id,
          invoiceNumber: inv.invoiceNumber,
          date: new Date(inv.dateCreation).toLocaleDateString("es-SV"),
  ...
  const handleConfirmDelete = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        toast.success(
          `Factura ${selectedPurchase.id} eliminada permanentemente`,
        );
      } else {
    ...
    <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-8">
      ...
      <div className="flex items-center justify-between mt-8 text-gray-400 text-sm">
        ...
        <div className="flex items-center gap-4">
          <span>
            1-{purchases.length} de {purchases.length}
          </span>
          <div className="flex gap-2">
            <button
              className="p-1.5 rounded bg-[#5BA4E1] text-white opacity-50 cursor-not-allowed"
              disabled
            >
              <ChevronLeft size={18} />
            </button>
            <button
              className="p-1.5 rounded bg-[#5-[#5BA4E1]] text-white opacity-50 cursor-not-allowed"
              disabled
            >
          ...
          <div className="bg-white rounded-xl shadow-xl w-full max-w-lg overflow-hidden animate-in fade-in zoom-in-95 duration-150">
            ...
            <div className="p-6">
              <div className="flex justify-between text-xs text-gray-400 mb-4 border-b border-gray-100 pb-2">
                <span>
                  Fecha: <b>{selectedPurchase.date}</b>
                </span>
                <span>
                  Descuento aplicado:{" "}
                  ...
                  <tbody>
                    ...
                    selectedPurchase.products.length > 0 ? (
                      selectedPurchase.products?.map((prod, index) => (
                        <tr key={index}>
                          <td className="p-2 font-mono text-gray-600 text-[11px]">
                      ...
                      <tr>
                        ...
                        >
                          Detalles de artículos no disponibles para esta
                          factura.
                        </td>
                      </tr>
                    )}
            ...
            <div className="bg-gray-50 px-6 py-3 border-t border-gray-100 flex justify-end">
              <button
                onClick={() => setIsViewOpen(false)}
                className="px-4 py-2 bg-sky-500 hover:bg-sky-600 text-white text-xs font-semibold rounded-lg shadow-sm transition-colors cursor-pointer"
              >
                Entendido
              </button>
            </div>
          ...
          <div className="bg-white rounded-xl shadow-xl w-full max-w-md overflow-hidden animate-in fade-in zoom-in-95 duration-150">
            <div className="p-6 flex gap-4 items-start">
              ...
              <div>
                ...
                <p className="text-xs text-gray-500 mt-2 leading-relaxed">
                  Estás a punto de remover la factura{" "}
                  <span className="font-mono bg-gray-100 px-1 py-0.5 rounded text-gray-700 font-bold">
                    {selectedPurchase.id}
                  </span>
                  . Esta acción impactará directamente la base de datos de
                  manera permanente.
                </p>
              </div>
            </div>
            ...
            <div className="bg-gray-50 px-6 py-3 border-t border-gray-100 flex justify-end gap-3">
              <button
                ...
                className="px-3 py-2 border border-gray-200 hover:bg-gray-100 text-gray-600 text-xs font-semibold rounded-lg transition-colors cursor-pointer"
              >
                Cancelar
              </button>
              <button
