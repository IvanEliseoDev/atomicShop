    ...
    loginEmployee: async (req: Request, res: Response) => {
        try {
            ...
            if (!isMatch) {
                ...
                await employee.save();
                return res.status(401).json({ status: 401, message: "Credenciales inválidas", data: null });
            }
            employee.loginAttemps = 0;
            ...
            const token = jwt.sign(
                //¿que vamos a guardar?
                { id: employee._id, userType: "employee", position:employee.position, email:employee.email},
                //#2- Secret key
                config.jwt.secret,
                //#3- cuando expira
                { expiresIn: "30d" },
