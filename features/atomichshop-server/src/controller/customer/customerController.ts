...
export const customerController = {
    ...
    getCustomerByID: async (req: Request, res: Response) => {
        try {
            const { id } = req.params;
            if (!id) {
                return res.status(400).json({status:400, message: "ID Customer is null" });
            }
            ...
            return res.status(200).json({ status: '200', message: "Customers find successfully", data: customer });
        } catch (error) {
            console.log(error)
            return res.status(500).json({ status: "500", message: "Internal Server Error - Check Server Logs" })
        }
